...
16 прогонов baseline vs skill, слепая оценка по критериям, зафиксированным до прогонов.

| | baseline | с навыком |
|---|---|---|
| `paylane` — способы оплаты, продиктованный фикс / унаследованный план | 3/5, 2/5 | **5/5, 5/5** |
| `deplock` — разрешение зависимостей, продиктованный фикс (3 повтора) | 4, 4, 5 | **5, 5, 5** |
| `deplock` — унаследованный план (3 повтора) | 4, 2, 4 | 4, 4, 3 |

Честное чтение: самый ясный эффект — **сходимость**. У арма с навыком нет слабых прогонов, у
baseline есть. На сценарии с унаследованным планом в `deplock` разделения нет, один
зафиксированный заранее критерий оказался негодным и был расщеплён, а шаги, которые происходят
после кода, не проверялись вовсе. Всё это разобрано в результатах.
